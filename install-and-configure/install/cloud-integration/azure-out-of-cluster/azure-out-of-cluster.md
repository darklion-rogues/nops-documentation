...
Connecting your Azure account to nOps allows you to view Kubernetes metrics side-by-side with out-of-cluster (OOC) costs (e.g. Azure Database Services). Additionally, it allows nOps to reconcile measured Kubernetes spend with your actual Azure bill. This gives teams running Kubernetes a complete and accurate picture of costs. For more information, read [Cloud Billing Integrations](/install-and-configure/install/cloud-integration/README.md) and this [blog post](https://blog.nOps.com/blog/complete-picture-when-monitoring-kubernetes-costs/).

To configure nOps's Azure Cloud Integration, you will need to set up daily exports of cost reports to Azure storage. nOps will then access your cost reports through the Azure Storage API to display your OOC cost data alongside your in-cluster costs.

A GitHub repository with sample files used in below instructions can be found [here](https://github.com/nOps/poc-common-configurations/tree/main/azure).
...
It will take a few hours to generate the first report, after which nOps can use the Azure Storage API to pull that data.
...
{% hint style="info" %}
For more granular billing data it is possible to [scope Azure cost exports](https://learn.microsoft.com/en-us/azure/cost-management-billing/costs/understand-work-scopes) to resource groups, management groups, departments, or enrollments. AKS clusters will create their own resource groups which can be used. This functionality can then be combined with nOps [multi-cloud](/install-and-configure/install/cloud-integration/multi-cloud.md) to ingest multiple scoped billing exports.
{% endhint %}

## Step 2: Provide access to Azure Storage API

Obtain the following values from Azure to provide to nOps. These values can be located in the Azure Portal by selecting _Storage Accounts_, then selecting your specific Storage account for details.
...
Next, upgrade nOps via Helm:

```bash
$ helm upgrade nOps nOps/cost-analyzer -n nOps -f values.yaml
```

You can verify a successful configuration by checking the following in the nOps UI:
...
* `$ kubectl get secrets -n nOps` to verify you've properly configured `cloud-integration.json`.
* `$ helm get values nOps` to verify you've properly set `.Values.nOpsProductConfigs.cloudIntegrationSecret`
...
*   The following error is reflective of nOps's previous Azure Cloud Integration method and can be safely disregarded.
