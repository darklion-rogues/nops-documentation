# Configure Keycloak Identity Provider for nOps
...
3. Select _Create_ to add nOps to the list of clients. Define a `clientID`. Ensure the Client Protocol is set to `openid-connect`.
4. Select your newly created client, then go to _Settings_.
   1. Set Access Type to `confidential`.
   2. Set Valid Redirect URIs to `http://YOUR_nOps_ADDRESS/model/oidc/authorize`.
   3. Set Base URL to `http://YOUR_nOps_ADDRESS`.

The [`.Values.oidc`](https://github.com/nOps/cost-analyzer-helm-chart/blob/721555b6641f72f2fd0c12f737243268923430e0/cost-analyzer/values.yaml#L194-L202) for Keycloak should be as follows:
...
oidc:
  ...
  # Find this in Keycloak UI by going to your nOps client, then clicking on "Credentials".
  ...
  loginRedirectURL: "http://YOUR_nOps_ADDRESS/model/oidc/authorize"
