...
There are three methods to use nOps with AMP:
...
The below guide provide a high-level overview of using AMP with nOps. The links above provide detailed instructions for each method.
...
nOps utilizes AWS SigV4 proxy to securely communicate with AMP. It enables password-less authentication using service roles to reduce the risk of exposing credentials.

### Federated architecture

To support the large-scale infrastructure (over 100 clusters), nOps leverages a [Federated ETL architecture](/install-and-configure/install/multi-cluster/federated-etl/federated-etl.md). In addition to Amazon Prometheus Workspace, nOps stores its data in a streamlined format (ETL) and ships this to a central S3 bucket. nOps's ETL data is a computed cache based on Prometheus's metrics, from which users can perform all possible nOps queries. By storing the ETL data on an S3 bucket, this integration offers resiliency to your cost allocation data, improves the performance and enables high availability architecture for your nOps setup.
...
```sh
kubectl logs -n nOps deployments/nOps-cost-analyzer cost-model |grep RunDiagnostic
```

Test to see if the nOps metrics are available using Grafana or exec into the nOps frontend to run a cURL against the AMP endpoint:
...
```
kubectl port-forward -n nOps svc/nOps-cost-analyzer 9090:9090
```
...
```sh
kubectl exec -i -t \
  deployments/nOps-cost-analyzer \
...
The below queries must return data for nOps to calculate costs correctly.
...
If you have set `nOpsModel.promClusterIDLabel`, you will need to change the query (`CLUSTER_ID`) to match the label (typically `cluster` or `alpha_eksctl_io_cluster_name`).

```bash
kubectl exec -i -t -n $nOps_NAMESPACE \
...
The value of `cluster_id` should match the value of `nOpsProductConfigs.clusterName`.
...
2. Verify nOps metrics are available in AMP:
