...
{% hint style="info" %}
This documentation should only be consulted when using nOps Cloud! For information about Alerts for self-hosted nOps, see [here](/using-nOps/navigating-the-nOps-ui/alerts.md).
{% endhint %}
...
Currently, only spend changes in allocation costs are supported in nOps Cloud. This alert type detects sudden changes in allocation spending (spending relating to any Kubernetes objects) when compared to a previous interval.

![nOps Cloud Alerts page](/images/kc-cloud-alerts.png)
...
* Recipients: Choose which platform(s) you want your alert sent through. nOps Alerts can be sent via Slack/Microsoft Teams webhook URLs, or by email. All three platforms do not need values provided for them, but may trigger global recipients if left blank (see below).
...
Cluster Status notifications, if enabled, provide notice if and when the cluster agent(s) installed have stopped reporting data to nOps Cloud. 
